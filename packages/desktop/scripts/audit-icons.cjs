...
function simulateAdaptiveForeground(subjectBounds) {
  return {
    ...subjectBounds,
    widthRatio: subjectBounds.widthRatio / ADAPTIVE_FOREGROUND_VISIBLE_SCALE,
    heightRatio: subjectBounds.heightRatio / ADAPTIVE_FOREGROUND_VISIBLE_SCALE,
    centerX: 0.5 + (subjectBounds.centerX - 0.5) / ADAPTIVE_FOREGROUND_VISIBLE_SCALE,
    centerY: 0.5 + (subjectBounds.centerY - 0.5) / ADAPTIVE_FOREGROUND_VISIBLE_SCALE,
  };
}
...
async function main() {
  ...
  if (!assets.androidAdaptiveForeground.subjectBounds) {
    throw new Error('Android adaptive foreground subject could not be measured.');
  }
  const adaptiveAlpha = assets.androidAdaptiveForeground.alphaBounds;
  const adaptiveSubject = assets.androidAdaptiveForeground.subjectBounds;
  const edgeDifference = adaptiveAlpha
    ? Math.max(
      Math.abs(adaptiveAlpha.pixels.minX - adaptiveSubject.pixels.minX),
      Math.abs(adaptiveAlpha.pixels.minY - adaptiveSubject.pixels.minY),
      Math.abs(adaptiveAlpha.pixels.maxX - adaptiveSubject.pixels.maxX),
      Math.abs(adaptiveAlpha.pixels.maxY - adaptiveSubject.pixels.maxY),
    )
    : Number.POSITIVE_INFINITY;
  if (!adaptiveAlpha
    || edgeDifference > 2) {
    throw new Error('Android adaptive foreground contains pixels outside the logo artwork.');
  }
  const adaptiveRenderedSubject = simulateAdaptiveForeground(
    adaptiveSubject,
  );
  assertMatchesDesktop(
    { ...assets.androidAdaptiveForeground, subjectBounds: adaptiveRenderedSubject },
    desktopSubject,
    'androidAdaptiveRendered',
  );
  if (assets.androidAdaptiveBackground.subjectBounds) {
    throw new Error('Android adaptive background must not contain logo artwork.');
  }
