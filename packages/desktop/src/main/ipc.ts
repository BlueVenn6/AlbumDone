...
import { getExistingThumbnailPath, getThumbnailCacheRoot } from './thumbnailCache';
...
function getThumbnailRoot(): string {
  return getThumbnailCacheRoot(app.getPath('userData'));
}
...
async function scanPhotoFolder(folderPath: string, options: PhotoScanOptions = {}): Promise<PhotoScanResult> {
  ...
  const processScanFile = async (
    ...
    try {
      ...
      const cachedThumbnailPath = unchanged
        ? getExistingThumbnailPath(cachedRow?.thumbnail_path)
        : null;
      if (cachedThumbnailPath) photo.thumbnailUri = toLocalFileUri(cachedThumbnailPath);
      ...
      return {
        photo,
        row: buildPhotoIndexRow(
          photo,
          stats,
          resolvedRoot,
          scannedAt,
          unchanged ? cachedRow : undefined,
          cachedThumbnailPath,
        ),
        ...(fingerprintCandidate ? { fingerprintCandidate } : {}),
      };
...
function loadCanvasModule(): {
  ...
  try {
    return require('@napi-rs/canvas') as ReturnType<typeof loadCanvasModule>;
  } catch (err) {
    logger.error('native', '@napi-rs/canvas failed to load', err);
    throw new Error(`@napi-rs/canvas failed to load: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
function photoFromIndexRow(row: PhotoIndexRow): ImportedPhoto {
  const thumbnailPath = getExistingThumbnailPath(row.thumbnail_path);
  const thumbnailUri = thumbnailPath ? toLocalFileUri(thumbnailPath) : undefined;
  return {
    id: row.id,
    path: row.file_path,
    uri: row.uri,
...
function buildPhotoIndexRow(
  ...
  existingThumbnailPath = getExistingThumbnailPath(existing?.thumbnail_path),
): PhotoIndexRow {
  ...
  return {
    ...
    thumbnail_path: existingThumbnailPath,
    thumbnail_size: existingThumbnailPath ? existing?.thumbnail_size ?? null : null,
...
function getCachedPhotoIndex(albumId: string): Map<string, PhotoIndexRow> {
  ...
  return new Map(rows.map((row) => {
    ...
    const rowThumbnailPath = getExistingThumbnailPath(row.thumbnail_path);
    const thumbnail = siblings.find(
      (candidate) => getExistingThumbnailPath(candidate.thumbnail_path) !== null,
    );
    const thumbnailPath = rowThumbnailPath
      ?? getExistingThumbnailPath(thumbnail?.thumbnail_path);
    return [
      row.file_path_key,
      {
        ...row,
