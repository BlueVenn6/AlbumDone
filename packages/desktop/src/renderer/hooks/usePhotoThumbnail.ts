...
export function usePhotoThumbnail(
  ...
  const lastAutoRetryUri = useRef<string | null>(null);
  ...
  const onLoadError = useCallback(() => {
    ...
    if (thumbnailUriCache.get(cacheKey) === src) {
      thumbnailUriCache.delete(cacheKey);
    }
    thumbnailNullCache.delete(cacheKey);
    setSrc(null);
    setReason('thumbnail_failed');
    if (lastAutoRetryUri.current !== src) {
      lastAutoRetryUri.current = src;
