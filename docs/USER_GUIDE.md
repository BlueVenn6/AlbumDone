...
- Use Windows 10 or Windows 11 on an Intel or AMD x64 computer for native
  support.
- Windows 11 ARM64 can run the current x64 release through Windows x64
  emulation. It has been tested on a Microsoft SQ2 device, but it is not an
  ARM64-native build.
- Windows 10 ARM64 and 32-bit Windows are not supported by the current x64
  release.
...
- Intel 或 AMD x64 电脑上的 Windows 10、Windows 11 为原生支持环境。
- Windows 11 ARM64 可通过 Windows x64 模拟运行当前版本；该版本已在
  Microsoft SQ2 设备上测试，但并非 ARM64 原生版本。
- 当前 x64 版本不支持 Windows 10 ARM64 和 32 位 Windows。
- 在整理或删除照片前，先备份重要照片。
